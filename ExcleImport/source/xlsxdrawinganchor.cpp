        ...
        */
        ...
        } else if (reader.name() == QLatin1String("grpSp")) // <xsd:element name="grpSp" type="CT_GroupShape"/>
        {
            ...
            loadXmlObjectGroupShape(reader);
                   ...
                   QLatin1String("graphicFrame")) // <xsd:element name="graphicFrame" type="CT_GraphicalObjectFrame"/>
        {
            //Graphic Frame
            m_objectType = GraphicFrame;
...
//{{ liufeijin
        ...
        if (!m_pictureFile.isNull()) {
            m_drawing->relationships()->addDocumentRelationship(QStringLiteral("/image"),
                                                                QStringLiteral("../media/image%1.%2").arg(
                                                                        m_pictureFile->index() + 1).arg(
                                                                        m_pictureFile->suffix()));
            ...
            writer.writeAttribute(QStringLiteral("r:embed"), QStringLiteral("rId%1").arg(
                    m_drawing->relationships()->count()));  //sp_blip_rembed  QStringLiteral("rId%1").arg(m_drawing->relationships()->count()) can't made new pic
            writer.writeAttribute(QStringLiteral("xmlns:r"), QStringLiteral(
