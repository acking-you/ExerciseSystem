        ...
        if (cell->hasFormula()) {
            if (cell->formula().formulaType() == CellFormula::NormalType) {
                ...
                if (!cell->formula().formulaText().isEmpty()) {
                    ...
                    QString newFormulaText = convertSharedFormula(rootFormulaText, rootCellRef,
...
//    }
        ...
        QSharedPointer<Cell> cell = QSharedPointer<Cell>(
                new Cell(value.toPlainString(), Cell::SharedStringType, fmt, this));
...
*/
    bool
    Worksheet::writeFormula(int row, int column, const CellFormula &formula_, const Format &format, double result) {
        Q_D(Worksheet);
                ...
                for (int c = range.firstColumn(); c <= range.lastColumn(); ++c) {
                    if (!(r == row && c == column)) {
                        if (Cell *cell = cellAt(r, c)) {
                            ...
                            QSharedPointer<Cell> newCell = QSharedPointer<Cell>(
                                    new Cell(result, Cell::NumberType, fmt, this));
...
//}}
    ...
    void
    WorksheetPrivate::saveXmlCellData(QXmlStreamWriter &writer, int row, int col, QSharedPointer<Cell> cell) const {
        Q_Q(const Worksheet);
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                ...
                if (data->linkType == XlsxHyperlinkData::External) {
                    ...
                    relationships->addWorksheetRelationship(QStringLiteral("/hyperlink"), data->target,
